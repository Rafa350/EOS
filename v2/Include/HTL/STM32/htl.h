...
namespace htl {
	...
	inline void setMask(T_ &data, T_ mask) {
		data |= mask;
	}

	template <typename T_>
	inline void setMask(volatile T_ &data, T_ mask) {
		data |= mask;
	}

	template <typename T_>
	inline void clrMask(volatile T_ &data, T_ mask) {
		data &= ~mask;
	}

	template <typename T_>
	inline void clrMask(T_ &data, T_ mask) {
		data &= ~mask;
	}

	template <typename T_>
	inline void wrtMask(volatile T_ &data, T_ clrMask, T_ setMask) {
		auto tmp = data;
		tmp &= ~clrMask;
		tmp |= setMask;
		data = tmp;
	...
	template <typename T_>
	inline void wrtMask(T_ &data, T_ clrMask, T_ setMask) {
		auto tmp = data;
		tmp &= ~clrMask;
		tmp |= setMask;
		data = tmp;
	...
	template <typename T_>
	inline void setBit(volatile T_ &data, uint8_t bit) {
		data |= 1 << bit;
	}

	template <typename T_>
	inline void setBit(T_ &data, uint8_t bit) {
		data |= 1 << bit;
	}

	template <typename T_>
	inline void clrBit(volatile T_ &data, uint8_t bit) {
		data &= ~(1 << bit);
	}

	template <typename T_>
	inline void clrBit(T_ &data, uint8_t bit) {
		data &= ~(1 << bit);
	}

	template <typename T_>
	inline bool isSetBit(volatile T_ &data, uint8_t bit) {
		return (data & (1 << bit)) != 0;
	}

	template <typename T_>
	inline bool isSetBit(T_ &data, uint8_t bit) {
		return (data & (1 << bit)) != 0;
	}

	template <typename T_>
	inline bool isClrBit(volatile T_ &data, uint8_t bit) {
		return (data & (1 << bit)) != 0;
	}

	template <typename T_>
	inline bool isClrBit(T_ &data, uint8_t bit) {
		return (data & (1 << bit)) != 0;
	}
}
