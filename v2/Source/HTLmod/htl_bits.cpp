...
template<typename T_>
void htl::setBits(T_ &var, unsigned bits) {
	var |= (T_) bits;
}

template<typename T_>
void htl::clearBits(volatile T_ &var, unsigned bits) {
	var &= ~((T_) bits);
}


template<typename T_>
void htl::clearBits(T_ &var, unsigned bits) {
	var &= ~((T_) bits);
}


template<typename T_>
bool htl::checkSetBits(volatile T_ &var, unsigned bits) {
	return (var & bits) == bits;
}

template<typename T_>
bool htl::checkSetBits(T_ &var, unsigned bits) {
	return (var & bits) == bits;
}


template<typename T_>
bool htl::checkClearBits(volatile T_ &var, unsigned bits) {
	return (var & bits) == 0;
}


template<typename T_>
bool htl::checkClearBits(T_ &var, unsigned bits) {
	return (var & bits) == 0;
}
